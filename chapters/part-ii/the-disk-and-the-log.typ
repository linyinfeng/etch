...
pub fn write(path: &Path, bytes: impl AsRef<[u8]>) -> Result<(), LpError> {
...
pub fn read(path: &Path) -> Result<String, LpError> {
...
pub fn read_ok(path: &Path) -> Result<Option<String>, LpError> {
    match read(path) {
...
pub fn read_bytes(path: &Path) -> Result<Vec<u8>, LpError> {
    let bytes = std::fs::read(path).map_err(|err| LpError::io(path, err))?;
    debug!(path = %path.display(), bytes = bytes.len(), "read a file");
    Ok(bytes)
}

pub fn read_bytes_ok(path: &Path) -> Result<Option<Vec<u8>>, LpError> {
...
pub fn entries(path: &Path) -> Result<Vec<std::fs::DirEntry>, LpError> {
    let entries = std::fs::read_dir(path)
        .map_err(|err| LpError::io(path, err))?

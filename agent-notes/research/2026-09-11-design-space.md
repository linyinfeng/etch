...
| 缩进（三层规则，已实现） | ① **文档层**：Typst 自己剔除 fenced block 的公共缩进（写在列表项里的 chunk 不会被整体带偏）；② **引用层**：引用行的缩进被逐行复制到展开内容上（noweb 语义）；③ **显示层**：weave 必须把引用行的缩进画出来，否则文档在骗人（曾漏掉，已修 + `run.sh` 回归） | ②照做；③照做；待办：`--no-indent`（等价 noweb `-L`）给缩进敏感语言逃生 |
...
2. **固定点可测**。自举的验收标准不是"看起来对"，而是种子对文档的展开与手写原型**逐字节一致**（今天叫 `seed tangle lp.typ`，见 `decisions/2026-09-11-final-shape.md`；文件在 2026-09-11 由 `bootstrap/`+`self.typ` 改名为 `seed/`+`lp.typ`）。这条测试同时守住了"重复 label 容忍"等未文档化行为。
3. **生成物不入库 + 需要种子**。既然 tangle 产物 gitignore，就必须保留一份**非工具生成**的可编译源码作为种子（今天可以是上一代产物，见 D19）；种子不是待维护的活代码，改动只进 `lp.typ`。

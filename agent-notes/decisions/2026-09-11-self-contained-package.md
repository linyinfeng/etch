...
- ⏳ 仍未做：`lp execute`、skill 的去留、用户侧 LSP 的便捷配置（用户明确推迟）。
...
- **`lp execute`**（在 `out/` 里跑命令，`flake` 不跟踪的前提）：未实现。它要解决的正是"nix 只认 git 里 tracked 的 flake"——放在 `out/` 里的 flake 只能靠 `path:` 或 `nix-shell` 读，而 `path:` 会把整个 `out/`（含 `target/`）拷进 store，所以更可能是 `lp execute` 里对这种"仓库外目录"做特殊处理。**先不做，先记着。**

...
- **仓库形态（D19 + D21）**：main 的树里只有 **四样**——`lp.typ`（文档）、`agent-notes/`、`README.md`（一行指针）、根上的 `.gitignore`。`AGENTS.md` 已删（一个名字就够，第二个名字只会漂）；`.gitignore` 是**反向 pattern**：`*` 全忽略，再 `!` 允许这三样（暂时还允许 `agent-notes/`，迁到自己的分支后就撤掉那两行）。其余一切——crate、包、示例、控制文件——都是 `lp tangle lp.typ` 的产物，默认写进文档旁边的 **`tangled/`**（`--out` 只在需要时写）。**seed 不再是目录，而是 `seed` 分支**（见下）。flake 与 skill 已于 2026-09-11 移除，`lp execute` 不做（D21）。

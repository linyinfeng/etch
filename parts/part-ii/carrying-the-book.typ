...
by `--check`. That is the one report a `.etchignore` could not have answered — the file was never unaccounted
...
data block, a PDF gets attached files, and `etch extract` reads either one back. What is left for this chapter

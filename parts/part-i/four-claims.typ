...
different program: the arrangement is not a report about the code, it changes what is cheap to write. The
arrangement is also the shape the argument takes once it is written down, which is what makes it maintainable
at all: what the author arranges is what a later reader — or a later machine — can go back and change, and the
code is what falls out of keeping it good. Knuth's observation from teaching WEB is that what is cheap to
write is what gets written well: in a conventional file, a procedure that updates a data structure has to
write its error recovery inline, and the recovery then looks like the bulk of the procedure, while an author
avoids that — error handling

...
- *One source, two outputs.* `etch tangle etch.typ` writes the crate, the package, the control files, and the copy
  of the book itself; `etch weave etch.typ etch.pdf` renders what you are reading.
...
`etch tangle --check` for drift, `etch map` for which declaration produced a line, `etch explain` for a compiler's

...
- `declared_files_are_accounted_for` — files and directories listed in `.etchignore` are left alone
...
- `without_a_declaration_a_stray_is_still_an_error` — with no `.etchignore` at all, strays are still errors
- `a_missing_output_directory_is_not_an_io_error` — a missing output file is drift, not an I/O failure
- `the_tools_own_scratch_is_not_content` — whatever a pass writes under `.etch` is never reported, because a killed run leaves it behind
...
fn tangled(declaration: &str, extra: &[(&str, &str)]) -> (TempDir, std::path::PathBuf) {
    ...
    if !declaration.is_empty() {
        ...
        std::fs::write(dir.path().join("out/.etchignore"), declaration).expect("ignore file");
    }
    ...
    let output = etch(&path, &["tangle", "doc.typ", "--out", "out"]);
...
fn a_dropped_declaration_is_an_error_until_it_is_resolved() {
    ...
    let report = etch(&dir, &["unaccounted", "doc.typ", "--out", "out"]);
    assert_eq!(report.status.code(), Some(1));
    assert!(stderr(&report).contains("src/b.py"), "{}", stderr(&report));

    let mut declaration = std::fs::read_to_string(dir.join("out/.etchignore")).expect("ignore");
    declaration.push_str("src/b.py\n");
    std::fs::write(dir.join("out/.etchignore"), &declaration).expect("ignore");
    let declared = etch(&dir, &["tangle", "doc.typ", "--out", "out"]);
    assert!(declared.status.success(), "{}", stderr(&declared));
    assert!(dir.join("out/src/b.py").exists(), "declared, so kept");

    std::fs::write(dir.join("out/.etchignore"), IGNORES).expect("ignore");
    let deleted = etch(
        &dir,
    ...
    let clean = etch(&dir, &["tangle", "doc.typ", "--out", "out"]);
    assert!(clean.status.success(), "{}", stderr(&clean));
    assert!(
        !dir.join("out/src/.etchmap.json").exists(),
        "the stale map is gone"
    );
    assert!(
        etch(&dir, &["tangle", "doc.typ", "--out", "out", "--check"])
            .status
            .success()
    );
}
...
fn a_deeper_ignore_file_can_take_a_file_back() {
    ...
    std::fs::write(dir.join("out/src/.etchignore"), "!stale.py\n").expect("ignore");
...
fn a_git_directory_is_ordinary_content() {
    ...
    std::fs::write(dir.path().join("out/.etchignore"), IGNORES).expect("ignore");
...
fn check_reports_a_stray_without_removing_it() {
    ...
    let output = etch(&dir, &["tangle", "doc.typ", "--out", "out", "--check"]);
...
fn the_stray_report_speaks_json() {
    ...
    let reported = etch(&dir, &["unaccounted", "doc.typ", "--out", "out"]);
...
fn the_tools_own_scratch_is_not_content() {
    let (_guard, dir) = tangled("", &[(".etch/leftovers/lib.typ", "the package")]);
    assert!(dir.join("out/.etch/leftovers/lib.typ").exists());
}
````)
...
fn a_missing_output_directory_is_not_an_io_error() {
    ...
    let output = etch(&path, &["tangle", "doc.typ", "--out", "out", "--check"]);
    assert!(!output.status.success());
    assert!(
...
fn a_refused_pass_writes_nothing() {
    ...
    let refused = etch(&dir, &["tangle", "doc.typ", "--out", "out"]);

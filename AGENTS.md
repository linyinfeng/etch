...
```
lp.typ              唯一真相：声明 Cargo.toml + src/*.rs + tests/*.rs + .agents/skills/**
seed/            冻结的种子 crate（tracked，永不重新生成）
Cargo.toml src/ tests/ .agents/   生成物（gitignore）——只改 lp.typ，不要手改这里
...
.agents/skills/       agent skill（也用 lp.typ 写）：LP 纪律 + lp 机制 + 一个跑得通的例子
```

常用命令：`nix develop -c cargo test`、`nix develop -c examples/demo/run.sh`（端到端）、`nix develop -c cargo clippy`。**fresh clone 先按 README 的"自举"三步**（`src/` 不在 git 里）：`cargo build --manifest-path seed/Cargo.toml` → `./seed/target/debug/lp tangle lp.typ --out .` → `cargo test`。
...
- **生成物不入库**：`examples/demo/build/` 之类一律 gitignore；CI 用 `lp tangle --check` 守漂移。`.typ` 是唯一真相。**工具自身的 crate 也是生成物**（ADR D15）：只改 `lp.typ`，手改 `src/` 会被 `tests/self.rs` 抓住；`seed/` 是冻结种子，不要跟着文档更新。
...
- **工具只管机制，论证归写作者**（ADR D18，它推翻了 D16）：声明**顺序自由**（骨架在前、或“先讲想法与片段、最后组装”，都正当）；fence 的 `lang` 只是数据（高亮/映射），不做一致性检查。工具强制的只有：引用可解析、无环、非空、`--check` 与文本一致、输出目录被解释（D10）。“重思路、渐进式披露、逻辑自洽”无法机械判定，靠 skill 的纪律（它第一句就说：文档的主语是**思想**，代码是思想的证据）。别把不可判定的性质换成可判定的代理当同一条规则（D16 就是这么推错的）。
- **转义**（ADR D17）：要原样输出 `<<name>>` 独占一行的样子，写成 `@<<name>>`（tangle 去掉 `@`，weave 渲染成字面量，不计入引用图）。文档在引用这套语法本身时用它——`lp.typ` 里的 skill 就是。
